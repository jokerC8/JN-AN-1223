...
OS_TASK(APP_taskZncb)
{
    ...
    if (OS_E_OK == OS_eCollectMessage(APP_msgZpsEvents, &sStackEvent)) {
        ...
        case ZPS_EVENT_APS_DATA_CONFIRM:
            memcpy(au8values,&sStackEvent.uEvent.sApsDataConfirmEvent.u8Status,sizeof(uint8));
            memcpy(&au8values[1],&sStackEvent.uEvent.sApsDataConfirmEvent.u8SequenceNum,sizeof(uint8));
            memcpy(&au8values[2],&sStackEvent.uEvent.sApsDataConfirmEvent.u8SrcEndpoint,sizeof(uint8));
            memcpy(&au8values[3],&sStackEvent.uEvent.sApsDataConfirmEvent.u8DstEndpoint,sizeof(uint8));
            vSL_WriteMessage(E_SL_APS_DATA_CONFIRM, 4, au8values);
        break;
#endif
#ifdef APP_APS_ACK
        case ZPS_EVENT_APS_DATA_ACK:
            memcpy(au8values,&sStackEvent.uEvent.sApsDataAckEvent.u8Status,sizeof(uint8));
            memcpy(&au8values[1],&sStackEvent.uEvent.sApsDataAckEvent.u8SequenceNum,sizeof(uint8));
            memcpy(&au8values[2],&sStackEvent.uEvent.sApsDataAckEvent.u8SrcEndpoint,sizeof(uint8));
            memcpy(&au8values[3],&sStackEvent.uEvent.sApsDataAckEvent.u8DstEndpoint,sizeof(uint8));
            memcpy(&au8values[4],&sStackEvent.uEvent.sApsDataAckEvent.u16ProfileId,sizeof(uint16));
            memcpy(&au8values[6],&sStackEvent.uEvent.sApsDataAckEvent.u16ClusterId,sizeof(uint16));
            vSL_WriteMessage(E_SL_APS_DATA_ACK, 8, au8values);
        break;
#endif
            ...
            case ZPS_EVENT_APS_DATA_INDICATION:
            {
                ...
                /* Child Aging */
                /*Request response event ? Call the child aging */
                if((ZPS_EVENT_APS_DATA_INDICATION == sStackEvent.eType) &&
                (0 == sStackEvent.uEvent.sApsDataIndEvent.u8DstEndpoint))
                    {
                    /* Check if this is a response from end device child, if so remove */
